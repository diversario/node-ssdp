...
function SsdpServer(opts) {
  SSDP.call(this, opts, 'ssdp-server')
}
...
SsdpServer.prototype.start = function () {
  ...
  this._logger.trace('Attempting to bind socket(s)')
...
SsdpServer.prototype._initAdLoop = function () {
  ...
  Object.keys(this.sockets).forEach(function (ipAddress) {
    var sock = self.sockets[ipAddress]
      , host = sock.address().address
      , port = sock.address().port

    self._logger.info('UDP socket bound', {host: host, port: port})
  })
...
SsdpServer.prototype.advertise = function (alive) {
  ...
  Object.keys(self._usns).forEach(function (usn) {
    ...
    self._send(message, 'Outgoing server message')
  })
}
