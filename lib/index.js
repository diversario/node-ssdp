 ...
 * @private
 */
SSDP.prototype._init = function (opts, _name) {
  opts.logger_name = _name
...
/**
 * Returns `true` if at least one socket is available.
 *
 * @returns {Boolean}
 * @private
 */
SSDP.prototype._hasSockets = function () {
...
/**
 * Creates a UDP4 socket for every available
 * non-internal IP interface and adds it to
 * the list of sockets.
 *
 * @returns {Array}
 * @private
...
SSDP.prototype._createSockets = function () {
  ...
  Object.keys(interfaces).forEach(function (iface) {
    self._logger.debug({'interface': iface}, 'Discovering IPs')

    interfaces[iface].forEach(function (ifInfo) {
      if (!ifInfo.internal && ifInfo.family.toLowerCase() == "ipv4") {
        delete sockets['dummy']
        self.sockets[ifInfo.address] = dgram.createSocket('udp4')
      }
    })
  })
}
...
SSDP.prototype._stop = function () {
  ...
  while (this._hasSockets()) {
    sock = this.sockets[Object.keys(this.sockets).pop()]
    sock.close()
    delete this.sockets[Object.keys(this.sockets).pop()]
  }
...
SSDP.prototype._start = function (port, host, cb) {
  ...
  Object.keys(this.sockets).forEach(function (ipAddress) {
    var sock = self.sockets[ipAddress]

    sock.on('error', function onSocketError(err) {
      self._logger.error(err, 'Socker error')
    })

    sock.on('message', function onSocketMessage(msg, rinfo) {
      self._parseMessage(msg, rinfo)
    })

    sock.on('listening', function onSocketListening() {
      ...
      function addMembership() {
        try {
          sock.addMembership(self._ssdpIp)
          sock.setMulticastTTL(self._ssdpTtl)
        } catch (e) {
          if (e.code === 'ENODEV') {
...
SSDP.prototype._respondToSearch = function (serviceType, rinfo) {
  ...
  Object.keys(self._usns).forEach(function (usn) {
    ...
    if (acceptor(usn, serviceType)) {
      ...
      self._send(message, peer, port, 'Sent M-SEARCH response')
    }
  })
}
...
SSDP.prototype._send = function (message, host, port, type) {
  var self = this
  ...
  Object.keys(this.sockets).forEach(function (ipAddress) {
    ...
    sock.send(message, 0, message.length, port, host, function (err, bytes) {
      self._logger.trace({
